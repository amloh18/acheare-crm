...
export class DevSeederPermissionsService {
  ...
  public async initPermissions({
    ...
    if (workspaceId === SEED_APPLE_WORKSPACE_ID) {
      if (light) {
        ...
        const existingGuestRole = await this.roleRepository.findOne(
          workspaceId,
          {
            where: { label: 'Guest' },
          },
        ...
        const guestRole =
          existingGuestRole ??
          (await this.roleService.createGuestRole({
            workspaceId,
  ...
  public async initMinimalPermissionsAndActivateWorkspace({
    ...
    let memberRole = (await this.roleRepository.findOne(workspaceId, {
      where: {
    ...
    if (!memberRole) {
      memberRole = await this.roleService.createMemberRole({
        workspaceId,
        ownerFlatApplication: workspaceCustomFlatApplication,
      });
    }
